...
class ArchiveSyncCoordinator {
  ...
  Future<void> _runSync({required bool force}) async {
    ...
    try {
      ...
      unawaited(_photoCacheStore.reconcilePrimaryPhotos(userId, snapshot.photos));
  ...
  Future<ArchiveSyncSnapshot> _fetchFullSnapshot(
    ...
    final collectibles = (results[1] as List)
        .map((row) => CollectibleModel.fromJson(Map<String, Object?>.from(row as Map)))
        .toList(growable: false);
    final wishlistItems = (results[2] as List)
        .map((row) => WishlistItemModel.fromJson(Map<String, Object?>.from(row as Map)))
        .toList(growable: false);
