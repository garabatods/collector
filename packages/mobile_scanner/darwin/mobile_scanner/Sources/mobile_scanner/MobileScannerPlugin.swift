...
private struct ComicSupplementCandidate {
    let value: String
...
public class MobileScannerPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, FlutterTexture, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    var comicSupplementOcrEnabled: Bool = false
            ...
            DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                ...
                do {
                    let barcodeRequest: VNDetectBarcodesRequest = VNDetectBarcodesRequest(completionHandler: { [weak self] (request, error) in
                        ...
                        let barcodeData = barcodes.map({ barcode -> [String: Any?] in
                            var data = barcode.toMap(
                                imageWidth: currentImage.width,
                                imageHeight: currentImage.height,
                                scanWindow: self.scanWindow
                            )
                            if let supplement = self.recognizeComicSupplement(
                                in: currentImage,
                                nextTo: barcode
                            ) {
                                data["supplementalValue"] = supplement.value
                                data["supplementalConfidence"] = Double(supplement.confidence)
                                data["supplementalSource"] = supplement.source
                            }
                            return data
                        })
                    ...
                    // UPC/EAN comic barcodes consist of a primary retail code
                    // plus a two- or five-digit supplemental code. Vision only
                    // attaches that supplement to the primary observation when
                    // composite symbologies are explicitly coalesced.
                    if #available(iOS 17.0, macOS 14.0, *) {
                        barcodeRequest.coalesceCompositeSymbologies = true
                    }
    ...
    private func recognizeComicSupplement(
        in image: CGImage,
        nextTo barcode: VNBarcodeObservation
    ) -> ComicSupplementCandidate? {
        guard comicSupplementOcrEnabled,
              let payload = barcode.payloadStringValue else {
            return nil
        }

        let baseDigits = payload.filter(\.isNumber)
        guard baseDigits.count == 12 || baseDigits.count == 13 else {
            return nil
        }

        if #available(iOS 17.0, macOS 14.0, *),
           let supplement = barcode.supplementalPayloadString,
           supplement.count == 5,
           supplement.allSatisfy(\.isNumber) {
            return ComicSupplementCandidate(
                value: supplement,
                confidence: 1,
        ...
        // The five-digit supplement is usually tiny and its position changes
        // with cover layout and camera orientation. Keep a small overlap with
        // the main barcode, then give Vision enough surrounding area to read
        // a supplement printed beside, above, or below it.
        let proposedRegion = CGRect(
            x: bounds.minX - (bounds.width * 0.18),
            y: bounds.minY - (bounds.height * 0.70),
            width: bounds.width * 1.36,
            height: bounds.height * 2.40
        )
        let region = proposedRegion.intersection(CGRect(x: 0, y: 0, width: 1, height: 1))
        guard !region.isNull, region.width > 0.02, region.height > 0.02 else {
            return nil
        }

        var bestCandidate: ComicSupplementCandidate?
        let textRequest = VNRecognizeTextRequest { request, _ in
            ...
            for observation in observations {
                for candidate in observation.topCandidates(3) {
                    guard let digits = self.comicSupplementDigits(
                    ...
                    if bestCandidate == nil || candidate.confidence > bestCandidate!.confidence {
                        bestCandidate = ComicSupplementCandidate(
                            value: digits,
    ...
    private func comicSupplementDigits(
        from text: String,
        baseDigits: String
    ) -> String? {
        ...
        // Vision can return the main UPC and the supplement as one recognized
        // string (with or without a separator). In that case the trailing five
        // digits are the supplement; accepting only an exact five-character
        // OCR result caused valid comic codes to be thrown away.
        guard digits.count == baseDigits.count + 5,
              digits.hasPrefix(baseDigits) else {
            return nil
        }
    ...
    func start(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        comicSupplementOcrEnabled = argReader.bool(key: "comicSupplementOcrEnabled") ?? false
    ...
    func analyzeImage(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        do {
            ...
            // Keep still-image analysis consistent with the live camera path
            // so comic UPC/EAN supplements are returned in the observation.
            if #available(iOS 17.0, macOS 14.0, *) {
                barcodeRequest.coalesceCompositeSymbologies = true
            }
...
extension VNBarcodeObservation {
    ...
    public func toMap(imageWidth: Int, imageHeight: Int, scanWindow: CGRect?) -> [String: Any?] {
        ...
        // Comic books commonly use an EAN/UPC barcode followed by a two- or
        // five-digit supplemental code that identifies an issue or cover.
        // Vision exposes that supplement separately, so combine it only for
        // the human-facing value while preserving `rawValue` as the base code.
        if #available(iOS 17.0, macOS 14.0, *),
           let payload = payloadStringValue,
           let supplement = supplementalPayloadString,
           !supplement.isEmpty {
            displayValue = payload + supplement
        }
